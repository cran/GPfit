...
wss_sum = c(sum(k[[1]]$withinss),sum(k[[2]]$withinss),sum(k[[3]]$withinss),sum(k[[4]]$withinss),sum(k[[5]]$withinss));
...
if (d >= 2) {
	param_wrap = matrix(c(0.20*(beta_range[2]-beta_range[1])+beta_range[1],0.5*(beta_range[2]-beta_range[1])+beta_range[1],0.8*(beta_range[2]-beta_range[1])+beta_range[1]),byrow=TRUE);
	...
	for (i in 1:nrow(param_wrap)){
		temp = optim(param_wrap[i],dev_wrapper,X=X,Y=Y,nug_thres=nug_thres,method="L-BFGS-B",lower=param_lower,upper=param_upper, control = c(maxit = maxit)); 
...
##---------------------------------------------------------##
	## Combineing the 2*d centers from the clusters with the single point from 
	## the diagonal search
	param_init = rbind(rep(dev[1,1],d),param_init);
}

#############################################################
## We now have the control[3]+1 initializing points for the optim search
dev_val = NULL;
for (i in 1:nrow(param_init)){
	temp = optim(param_init[i,],GP_deviance,X=X,Y=Y,nug_thres=nug_thres,method="L-BFGS-B",lower=param_lower,upper=param_upper, control = c(maxit = maxit));
...
GP$correlation_param = list(type = "exponential", power = 2)
